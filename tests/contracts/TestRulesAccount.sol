...
contract TestRulesAccount is IAccount, IPaymaster, Stakable {
    ...
    function setCoinState(uint arg) public {
        coin.pause(arg);
    }
    ...
    function runRule(string memory rule) public returns (uint) {
        if (eq(rule, "")) return 0;
        ...
        else if (eq(rule, "BALANCE"))
            return uint160(address(msg.sender).balance);
            ...
            return new Dummy{salt: bytes32(uint(0x1))}().value();
        else if (eq(rule, "SELFDESTRUCT")) {
            coin.destruct();
            return 0;
        } else if (eq(rule, "no_storage")) return 0;
        else if (eq(rule, "account_storage")) return state;
        else if (eq(rule, "account_reference_storage"))
            return coin.balanceOf(address(this));
        else if (eq(rule, "account_reference_storage_struct"))
            return coin.getInfo(address(this)).c;
        else if (eq(rule, "account_reference_storage_init_code"))
            return coin.balanceOf(address(this));
        else if (eq(rule, "external_storage"))
